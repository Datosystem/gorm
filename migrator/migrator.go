...
func (m Migrator) AutoMigrate(values ...interface{}) error {
	for _, value := range m.ReorderModels(values, true) {
		// Custom
		if t, ok := value.(TableModel); !ok || t.TableName()[0:1] == "(" {
			continue
		}
...
func (m Migrator) MigrateColumn(value interface{}, field *schema.Field, columnType gorm.ColumnType) error {
	...
	// check default value
	if !field.PrimaryKey {
		dbDefaultValue, _ := columnType.DefaultValue()
		fieldDefaultValue := field.DefaultValue

		// removes final parenthesis like in getDate()
		dbDefaultValue = strings.TrimSuffix(dbDefaultValue, "()")
		fieldDefaultValue = strings.TrimSuffix(fieldDefaultValue, "()")

		// removes wrapping parenthesis like in (1)
		dbDefaultValue = strings.TrimSuffix(strings.TrimPrefix(dbDefaultValue, "("), ")")
		fieldDefaultValue = strings.TrimSuffix(strings.TrimPrefix(fieldDefaultValue, "("), ")")

		if strings.EqualFold(fieldDefaultValue, "NULL") {
			fieldDefaultValue = ""
		}

		if !strings.EqualFold(dbDefaultValue, fieldDefaultValue) {
			alterColumn = true
		}
	}
